...
class ParticleFilter {
 public:
  ...
  float AngleDelta(float curr_angle, float prev_angle);
 ...
 private:
  ...
  struct Particle SampleMotionModel(Eigen::Vector2f odom_loc, float odom_angle, struct Particle particle);

  struct Particle NoiseFreeNextLocation(const Eigen::Vector2f& odom_loc, const float odom_angle, struct Particle p);

  std::tuple<float, float, float> ComputeDeltaBaseLink(const Eigen::Vector2f& odom_loc, const float odom_angle);

  geometry::line2f BuildLineSegment(Eigen::Vector2f lidar_loc, float angle, float range_min, float range_max);
};
}  // namespace slam
